...
type SearchResults struct {
	Result_rank  int    `json:"result_rank"`
	Result_URL   string `json:"result_url"`
	Result_title string `json:"result_title"`
	Result_Desc  string `json:"result_desc"`
}
...
func RandUserAgent() string {
	...
	return UserAgents[randNum] // selecting a random user from the given list
}

func buildGoogleURL(searchTerm, languageCode, countryCode string, pages, count int) ([]string, error) {
	// returning the link that we will be using for hitting request
	toScrape := []string{}
	// Refining the search term
	searchTerm = strings.Trim(searchTerm, " ")
	searchTerm = strings.Replace(searchTerm, " ", "+", -1)
	// searching the country code, from the map provided
...
func googleResultParsing(response *http.Response, rank int) ([]SearchResults, error) {
	// We have created a document, so that we can query the document, in order to find the needed entities
	body := response.Body
	...
	// This contains, every request
	sel := doc.Find("div.g")
	rank++
	// These nodes have the individual results that we need
...
func GoogleScrape(searchTerm, languageCode, countryCode string, proxyString interface{}, pages, count, waitTime int) ([]SearchResults, error) {
	results := []SearchResults{}
	resCount := 0 // no of searchURLs that we have found, ser
	googlePages, err := buildGoogleURL(searchTerm, languageCode, countryCode, pages, count)
	if err != nil {
		return nil, err
	}
	// We will be iterating over the URLs,
	for _, page := range googlePages {
		resp, err := scrapeClientRequest(page, proxyString)
		if err != nil {
			return nil, err
		}
		data, err := googleResultParsing(resp, resCount)
		if err != nil {
			return nil, err
		}
...
// simply, we check if the URL is getting through if not we return with a error, otherwise we return response
func scrapeClientRequest(searchURL string, proxyString interface{}) (*http.Response, error) {
	// if there will be something in proxy then it will create the baseClient otherwise it will continue with the default baseClient
	baseClient := getScrapeClient(proxyString)
	req, _ := http.NewRequest("GET", searchURL, nil)
	req.Header.Set("User-Agent", RandUserAgent())
	res, err := baseClient.Do(req)
	if res.StatusCode != 200 {
		fmt.Errorf("Scrapper received a non-200 status code\nBan Suggested")
		return nil, err
	}
	if err != nil {
		return nil, err
...
func main() {
	// Term goes like: GoogleScrape("searchterm", "languagecode", "countrycode", "page", "searchResultsperpage")
	res, err := GoogleScrape("aman", "en", "in", nil, 1, 30, 5)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
